...
public abstract class ModGameTypeResolver : IModGameTypeResolver
{
    ...
    public bool TryGetGameType(DetectedModReference modInformation, out ReadOnlyFrugalList<GameType> gameTypes)
    {
        ...
        Logger?.LogTrace("Try getting game type for '{Id}'", modInformation.ModReference.Identifier);
        var result = TryGetGameTypeCore(modInformation, out gameTypes);
        Logger?.LogDebug("Detected game types '{Id}': Success: {Result}; Types: [{Types}]", modInformation.ModReference.Identifier, result, string.Join("", gameTypes));
        return result;
    }
    ...
    {
        Logger?.LogTrace("Checking if '{Id}' is not compatible to {GameType}.", modInformation.ModReference.Identifier, expectedGameType);
    ...
    {
        Logger?.LogTrace("Checking if modinfo '{Name}' is not compatible to {GameType}.", modinfo?.Name, expectedGameType);
        // If the type resolver was unable to find the type, we have to assume that the current mod matches to the game.
        // Otherwise, we'd produce false negatives. Only if the resolver was able to determine a result, we use that finding.
