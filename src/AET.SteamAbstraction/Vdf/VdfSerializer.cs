...
internal class VdfSerializer
{
    ...
    {
        _settings = settings;
        if (_settings is { UsesConditionals: true, DefinedConditionals: null })
    ...
    private VConditional ReadConditional(VdfReader reader)
    {
        ...
        {
            if (reader.Value == "!")
                result.Add(new VConditional.Token(VConditional.TokenType.Not));
            else if (reader.Value == "&&")
                result.Add(new VConditional.Token(VConditional.TokenType.And));
            else if (reader.Value == "||")
                result.Add(new VConditional.Token(VConditional.TokenType.Or));
            else
